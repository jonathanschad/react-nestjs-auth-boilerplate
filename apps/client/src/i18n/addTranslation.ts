...
const saveKey = async (key: string) => {
	for (const lang in locales) {
		translations[lang] = (await lineReaderQuestionPromise(
			`Enter Translation for ${lang}: `,
		)) as string;
	}
	let summary = `\n\nKey: ${key} \n`;
	...
	rl.question("Is your input correct? (y/n): ", async (confirm: string) => {
		if (
			...
			for (const lang in translations) {
				...
				fs.writeFileSync(
					`./${lang}/${filename}.json`,
					JSON.stringify(orderKeys(locales[lang][filename])),
				);
			}
		}
		rl.close();
...
const enterNewTranslation = async () => {
	rl.question("\nEnter new Translation key: ", async (key: string) => {
		...
		if (keyInFiles.includes(filename as unknown as string)) {
			...
			console.log(
				"\n The key already exists in the following file(s): ",
				keyInFiles.toString(),
			);
	...
	if (file !== "addTranslation.js" && stats.isDirectory()) {
		...
		fs.readdirSync(`./${file}/`).forEach((fileDir: string) => {
			if (fileDir.endsWith(".json")) {
				...
				locales[file][fileDir] = JSON.parse(
					fs.readFileSync(`./${file}/${fileDir}.json`, "utf8"),
				);
			}
		});
	}
...
console.log(
	"Enter the number if the translationfile or enter the name of a new one \n",
);
...
rl.question("\nNumber or new filename: ", async (filenumberLocal: string) => {
	if (Number.isNaN(parseInt(filenumberLocal))) {
		filename = filenumberLocal;
		rl.question(
			`Are you sure you want to create file "${filenumberLocal}.json?" (y/n) `,
			async (yesNo: string) => {
		...
		if (filename) {
			...
			for (const lang in locales)
				if (!locales[lang][filename]) locales[lang][filename] = {};
			await enterNewTranslation();
		} else {
			console.log("\n Invalid number");
			rl.close();
