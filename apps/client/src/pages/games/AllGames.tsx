import type { EloHistoryEntityApiDTO, GameEntityApiDTO } from '@boilerplate/types';
...
import { useGetGames, useGetGamesCount } from '@/api/dart/game/useGetGames';
import {
...
export const AllGames = () => {
    useSetSignedInCurrentActiveRoute(CurrentlySelectedRouteOptions.GAMES);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const pageSize = 10;

    const { data, isLoading, error } = useGetGames({}, page, pageSize);

    const { data: countData, isLoading: countLoading, error: countError } = useGetGamesCount({});
    ...
    if (error || !data || countError || !countData) {
        return (
            <div className="mx-auto flex h-full w-full max-w-6xl flex-col gap-6">
                <Card className="p-4">
                    <Typography as="p">
                        <Translation>errorLoadingAllGames</Translation>
                    </Typography>
                </Card>
            </div>
        );
                    ...
                    <>
                                ...
                                <TableBody>
                                    {table.getRowModel().rows.map((row) => (
                                        <TableRow
                                            ...
                                            onClick={() => navigate(`/games/${row.original.id}`)}
                                        >
                        ...
                        <div className="mt-4 flex items-center justify-between">
                            <Typography as="smallText" className="text-muted-foreground">
                                ...
                                {totalPages} ({countData.count} <Translation>totalGames</Translation>)
                            </Typography>
